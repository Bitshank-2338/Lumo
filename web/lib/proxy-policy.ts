...
const STATIC_ASSET = /\.(?:png|jpe?g|gif|svg|ico|webp|avif|woff2?|ttf|otf|txt|json|map|css|js)$/i

const PUBLIC_SURFACES = ['/docs', '/hub', '/source']
...
export function isAuthExempt(pathname: string): boolean {
  return (
    pathname === '/' ||
    PUBLIC_SURFACES.some(route => pathname === route || pathname.startsWith(`${route}/`)) ||
...
export function classifyToken(token: string | undefined, nowMs: number): TokenState {
