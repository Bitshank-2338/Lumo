...
test('isAuthExempt allows the public Lumo website', () => {
  assert.equal(isAuthExempt('/'), true)
  assert.equal(isAuthExempt('/docs'), true)
  assert.equal(isAuthExempt('/docs/getting-started'), true)
  assert.equal(isAuthExempt('/hub'), true)
  assert.equal(isAuthExempt('/source'), true)
})
...
test('isAuthExempt does NOT exempt protected app routes', () => {
  ...
  assert.equal(isAuthExempt('/hub-secret'), false)
})
